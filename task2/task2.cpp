...
void Task2::initTweakBar()
{
    ...
    TwAddVarRW(bar, "Light power", TW_TYPE_FLOAT, &power, "min=1 max=5 step=0.1");
    ...
    TwAddVarRW(bar, "SpecularPower ", TW_TYPE_FLOAT, &specular_power, "min=1 max=100 step=0.1");
}
...
{
    handleWireFrame();
    handleColorFrame(time_from_start, zoomValue);
...
void Task2::handleColorFrame(float time_from_start, float zoom)
{
    ...
    zoomValue += zoom;
    zoomValue = zoomValue <= 1 ? 0.1 : zoomValue;

    mat4 const view = lookAt(vec3(0, 0, zoomValue), vec3(0, param, 0), vec3(0, 1, 0));
